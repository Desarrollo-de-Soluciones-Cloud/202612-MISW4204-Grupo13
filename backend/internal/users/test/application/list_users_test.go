...
func TestListUsersWithData(t *testing.T) {
	mockRepo := newMockUserRepository()
	mockRepo.byID[1] = &domain.User{ID: 1, Name: "John Doe", Email: "john@example.com", Password: "password-hash", GlobalRole: domain.RoleProfessor}
	mockRepo.byID[2] = &domain.User{ID: 2, Name: "Jane Doe", Email: "jane@example.com", Password: "password-hash", GlobalRole: domain.RoleAdmin}
