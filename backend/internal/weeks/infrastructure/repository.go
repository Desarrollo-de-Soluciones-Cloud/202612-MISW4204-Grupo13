...
func (r *WeekRepository) FindByPeriodIDAndNumber(periodID uint, number int) (*domain.Week, error) {
	var week domain.Week
	result := database.DB.Where("period_id = ? AND number = ?", periodID, number).First(&week)
	if result.Error != nil {
